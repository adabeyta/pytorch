...
def my_from_file(filename, shared=None, size=0, dtype=None) -> Tensor:
    """
    Stable ``from_file`` (requires build time 2.15+). Creates a CPU tensor
    backed by the contents of a file.
    ...
    """
    return torch.ops.libtorch_agn_2_10.my_from_file.default(
        filename, shared, size, dtype
    )
